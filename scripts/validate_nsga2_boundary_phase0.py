...
def _load_base_helpers():
    ...
    spec = importlib.util.spec_from_file_location("_candidate_suite_boundary_phase0_base", helper_path)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Phase 0 sanity validation for boundary-preservation candidate_m."
    )
    parser.add_argument("--config", default="configs/smoke/zdt1_nsga2_smoke.json")
    parser.add_argument("--problems", default="zdt1")
    parser.add_argument("--output-root", default="outputs/nsga2_boundary_phase0")
    parser.add_argument("--artifact-root", default="artifacts")
    parser.add_argument("--artifact-suffix", default=None)
    parser.add_argument("--seeds", type=int, default=3)
    parser.add_argument("--seed-start", type=int, default=10301)
    parser.add_argument("--budget", type=int, default=300)
    parser.add_argument("--include-candidate-l", action="store_true")
    parser.add_argument("--include-pymoo", action="store_true")
    parser.add_argument("--include-deap", action="store_true")
    parser.add_argument("--include-random-archive", action="store_true")
    return parser.parse_args()


def _phase0_variants(include_candidate_l: bool) -> list[NSGA2CandidateVariant]:
    variants: list[NSGA2CandidateVariant] = [
        candidate_j_h_lite_retry2(),
        candidate_m_boundary_preservation_light(),
    ]
    if include_candidate_l:
...
def _sanity_gate_results(
    ...
    candidate_rows = [row for row in raw_rows if row["algorithm"] == "candidate_m_boundary_preservation_light"]
    ...
    execution_ok = bool(candidate_rows) and all(row.get("success") for row in candidate_rows)
    ...
    return [
        {
            ...
            "note": "candidate_m metadata present with expected base candidate and mechanism",
        },
        {
            "gate": "default_changed=false",
            "result": default_changed_ok,
            "pass_fail": default_changed_ok,
            "note": "candidate_m metadata keeps default_changed=false",
        },
        {
        ...
        },
        {
            ...
            "note": "candidate_m actual evaluations match requested budget",
        },
        {
        ...
        {
            "gate": "candidate_m execution",
            "result": execution_ok,
            "pass_fail": execution_ok,
            "note": "candidate_m run succeeded on all selected seeds",
        },
        {
            "gate": "catastrophic regression 여부",
...
def main() -> None:
    ...
    variants = _phase0_variants(args.include_candidate_l)
    ...
    report_path = safe_artifact_path(
        artifact_root,
        "nsga2_boundary_preservation_phase0_report",
        args.artifact_suffix,
        ".md",
    )
    ...
    md_lines = [
        "# NSGA-II Boundary Preservation Phase 0 Results",
        "",
        "## Aggregate Results",
        "",
    ...
    report_lines = [
        "# NSGA-II Boundary Preservation Phase 0 Report",
        "",
        "## 1. Executive Summary",
        "",
        "- 이번 작업의 목표: candidate_m_boundary_preservation_light의 Phase 0 sanity 확인",
        "- 구현한 candidate: `candidate_m_boundary_preservation_light`",
        "- 기본값 변경 여부: `false`",
        ...
        "",
        "- Scope: candidate_m_boundary_preservation_light, Phase 0 sanity, ZDT1 small run, candidate isolation, fairness check, artifact generation",
        "- Non-Scope: default promotion, candidate_m change request, approved opt-in profile, Phase 1 full benchmark, DTLZ/WFG validation, new survivor-pressure families, productization",
        "",
        "## 3. Candidate Definition",
        "",
        *BASE._markdown_table(
            [
                {"field": "candidate_id", "value": "candidate_m_boundary_preservation_light"},
                {"field": "base_candidate", "value": "candidate_j_h_lite_retry2"},
                {"field": "mechanism", "value": "boundary_preservation_light"},
        ...
        *BASE._markdown_table(
            [
                {
                    "file": "src/ga_lab/algorithms/nsga2.py",
                    "change": "added boundary_preservation_light as an opt-in partial-front strategy",
                    "default_impact": "none when strategy is absent",
                },
                {
                    "file": "src/ga_lab/experiment/nsga2_candidate_variants.py",
                    "change": "added candidate_m metadata and apply logic",
                    "default_impact": "none",
                },
                {
                    "file": "configs/candidates/nsga2_boundary_preservation_candidate_m.json",
                    "change": "registered candidate_m config",
                    "default_impact": "none",
                },
                {
                    "file": "scripts/validate_nsga2_boundary_phase0.py",
                    "change": "added dedicated boundary Phase 0 runner",
                    "default_impact": "none",
                },
            ],
        ...
        *BASE._markdown_table(
            [
                ...
                {
                    "item": "algorithms",
                    "value": ", ".join(["internal_nsga2", *[variant.candidate_id for variant in variants]]),
                },
