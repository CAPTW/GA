...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run budget-matched pure GA vs hybrid GA vs baseline comparisons."
    )
    parser.add_argument(
        "--manifest",
        action="append",
        required=True,
        help=(
            "Hybrid comparison manifest path. "
            "Pass multiple times to merge suites into one summary."
        ),
    )
    parser.add_argument(
        "--output-root",
    ...
    parser.add_argument(
        "--summary-stem",
        default="hybrid_comparison_summary",
