...
if (-not $env:GA_LAB_SCHEDULER_JOBS_PATH) {
    $env:GA_LAB_SCHEDULER_JOBS_PATH = Join-Path $projectRoot "configs\schedules\default_jobs.json"
}
...
$summary = @{
    ...
    scheduler_jobs_path = if ($JobsConfig) { $JobsConfig } else { $env:GA_LAB_SCHEDULER_JOBS_PATH }
...
$args = @("scripts/run_ops_scheduler.py")
if ($JobsConfig) { $args += @("--jobs-config", $JobsConfig) }
if ($Once) { $args += "--once" }
if ($RegisterOnly) { $args += "--register-only" }
if ($JobName) { $args += @("--job-name", $JobName) }
