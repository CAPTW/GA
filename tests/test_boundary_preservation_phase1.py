...
def test_boundary_phase1_runner_writes_candidate_m_artifacts(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            "scripts/validate_nsga2_boundary_phase1.py",
            "--problems",
            "zdt1,zdt2,zdt3",
            "--seeds",
            "1",
    ...
    assert (
        results_json.name
        == "nsga2_boundary_preservation_phase1_results_boundary_phase1_test.json"
    )
    assert (
        report_md.name
        == "nsga2_boundary_preservation_phase1_report_boundary_phase1_test.md"
    )
