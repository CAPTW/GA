...
class ZDT3Problem:
    ...
    def __init__(self, name: str = "zdt3") -> None:
        self.name = name

    def fitness(self, genome: Genome) -> Fitness:
        if len(genome) < 2:
            raise ValueError("ZDT3 requires at least 2 real genes")
        f1 = genome[0]
        if f1 < 0:
            f1 = 0.0
        if f1 > 1:
    ...
    def hypervolume_reference_point(self, directions: list[bool]) -> list[float]:
        ...
        if len(directions) != len(bounds):
            raise ValueError("directions length must match ZDT3 objective count")
